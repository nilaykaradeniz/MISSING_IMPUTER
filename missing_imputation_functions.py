...
def missing_imputation(dataframe,num_col=True,cat_col=True):
    ...
    return fillna_df
fillna_df=missing_imputation(df)
...
mean_df,df=MeanMedianImputerLibrary(df)
...
def ArbitraryNumberImputerLibrary(dataframe,constant_number=-999,constant=False,dict_constant=True):
    ...
    return dataframe
df=ArbitraryNumberImputerLibrary(df)
...
cat_df,df=CategoricalImputerLibrary(df)
...
tail_df,df=EndTailImputerLibrary(df)
...
def RandomSampleImputerLibrary(dataframe,seed="observation",seeding_method='add'):
    ...
    if question == "YES":
        ...
        print("Wrong information...")
    return dataframe
df = RandomSampleImputerLibrary(df)
...
binary_na_columns,df = AddMissingIndicatorLibrary(df)
...
def DropMissingDataLibrary(dataframe,null_high_col,null_high=False):
    ...
    return dataframe
df = DropMissingDataLibrary(df,null_high_col_name)
