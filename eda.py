...
def csv_file(file_name,refresh=False):
    ...
    gc.collect()
    if refresh:
        dataframe_csv.to_pickle(os.getcwd() + "/pickle_dataset/" + file_name + ".pkl")
...
def excel_file(file_name,sheet,refresh=False):
    ...
    gc.collect()
    if refresh:
        dataframe_xlsx.to_pickle(os.getcwd() + "/pickle_dataset/" + file_name + ".pkl")
...
def desc_statistics(dataframe,num_cols,cat_cols,head=True,count=5,shape=True,dtypes=True,
    ...
    null_high_col_name={}
    if high_null_count:
        print("TOTAL NUMBER OF MISSING VALUES AND RATIO")
        print("TOTAL NUMBER OF MISSING VALUES :", dataframe.isnull().sum().sum())
